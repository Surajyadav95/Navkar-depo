...
Namespace Resource
    Namespace DataAccessLayer
        Public Class SQLDatabaseOperations
            ...
            Public Shared Function ExecuteReader1(ByVal commandType As CommandType, _
                                                    ByVal commandText As String, ByVal ParamArray cmdParameters As SqlParameter()) As Object
                'if we receive parameter values, we need to figure out where they go
                'create a command and prepare it for execution
                Dim connectionString As String = ConfigurationManager.AppSettings("SqlConnString")
                ...
                Command.CommandText = commandText
                'if we were provided a transaction, assign it.
                Command.Transaction = CType(Nothing, SqlTransaction)
                'set the command type
                Command.CommandType = commandType
                'attach the command parameters if they are provided
                If Not (cmdParameters Is Nothing) Then
                    AttachParameters(Command, cmdParameters)
                End If
                'execute the command & return the results
                gstReader = Command.ExecuteReader()
                'detach the SqlParameters from the command object, so they can be used again
            ...
            'new xmlreader
            Public Shared Function ExecuteXmlReader(ByVal connectionString As String, _
                                                       ByVal commandType As CommandType, _
                                                       ByVal commandText As String, _
                                                       ByVal ParamArray parameterValues() As Object) As Object
                Dim commandParameters As SqlParameter()
                ...
                Dim gstCon As New SqlConnection(connectionString)
                Dim gstCommand As New SqlCommand
                Dim gstReader As System.Xml.XmlReader
                PrepareCommand(gstCommand, gstCon, CType(Nothing, SqlTransaction), commandType, commandText, commandParameters)
                'execute the command & return the results
                gstReader = gstCommand.ExecuteXmlReader()
                'detach the SqlParameters from the command object, so they can be used again
                gstCommand.Parameters.Clear()
                Return gstReader
            End Function 'ExecuteReader
            'new function for get data reader
        End Class
    End Namespace
End Namespace
