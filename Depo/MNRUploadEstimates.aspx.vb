...
Partial Class Summary_BCYMovement
    ...
    Private Sub sub_CreateTable()
        ...
        dtDepoContainer.Columns.Add("EstimateDate")
        dtDepoContainer.Columns.Add("LabourAmount")
        dtDepoContainer.Columns.Add("MaterialAmount")
        dtDepoContainer.Columns.Add("CleaningAmount")
        dtDepoContainer.Columns.Add("TotalEstimateAmount")
        dtDepoContainer.Columns.Add("ApprovedAmount")
        dtDepoContainer.Columns.Add("ApprovedDate")
        dtDepoContainer.Columns.Add("RepairedDate")
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            ...
            param.ParameterName = "@PT_MNRESTIMATEDOWNLOAD"
            param.Value = dtDepoContainer
            param.TypeName = "PT_MNRESTIMATEDOWNLOAD"
            ...
            cmd.CommandText = "USP_SAVE_ESTIMATES_DOWNLOAD"
            cmd.Connection = con
    ...
    Protected Sub lnkDownloadExcel_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += "Select '' [Container No],'' [Size/Type],'' [Estimate Date (yyyy-MM-dd)],'' [Labour Amount],'' [Material Amount],'' [Cleaning Amount],'' [Total Estimate Amount],'' [Approved Amount],'' [Approved Date (yyyy-MM-dd)],'' [Repaired Date (yyyy-MM-dd)]"
            dt = db.sub_GetDatatable(strSql)

            If (dt.Rows.Count > 0) Then
                Using wb As New XLWorkbook()
                    wb.Worksheets.Add(dt, "MNR Estimate Download Template")
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename=MNREstimateTemplate.xlsx")
                    ...
                    For Each row As IXLRow In workSheet.Rows()
                        If Not Trim(row.Cell(1).Value.ToString()) = "" Then
                            If Not firstRow Then
                                ...
                                dtRow.Item("EstimateDate") = Convert.ToDateTime(row.Cell(3).Value).ToString("yyyy-MM-dd")
                                dtRow.Item("LabourAmount") = row.Cell(4).Value.ToString()
                                dtRow.Item("MaterialAmount") = row.Cell(5).Value.ToString()
                                dtRow.Item("CleaningAmount") = row.Cell(6).Value.ToString()
                                dtRow.Item("TotalEstimateAmount") = row.Cell(7).Value.ToString()
                                dtRow.Item("ApprovedAmount") = row.Cell(8).Value.ToString()
