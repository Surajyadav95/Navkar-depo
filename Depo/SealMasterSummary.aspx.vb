...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            ...
            End If
            strSql = ""
            strSql += "SP_SealEntryInventory '" & Convert.ToDateTime(Trim(txtfromDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(Trim(txttoDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(ddlLineName.SelectedValue) & "'"
        ...
        Try
            For Each row As GridViewRow In grdRegistrationSummary.Rows
                ...
                If (chkright.Checked = True) Then
                    strSql = ""
                    strSql = "select 'Seal No Out' as msg from EYardEmptyOut where SealNo='" & Trim(CType(row.FindControl("lblassessNo"), Label).Text & "") & "' "
                    ...
                    End If
                    strSql = ""
                    strSql += "update Import_SealMaster  set Iscancel=1 where  PrefixNo='" & Trim(CType(row.FindControl("lblassessNo"), Label).Text & "") & "'"
                    db.sub_ExecuteNonQuery(strSql)
                End If
            Next
            btnSave_Click(sender, e)
            lblSession.Text = "Seal Cancelled Successfully"
