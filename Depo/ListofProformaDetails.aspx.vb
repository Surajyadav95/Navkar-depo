...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnsearch_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " USP_Handling_PROFORMA_summary '" & Convert.ToDateTime(txtfromDate.Text).ToString("yyyy-MM-dd 00:00:00 ") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("yyyy-MM-dd 23:59:00") & "',"
    ...
    Protected Sub lnkCancel_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "UPDATE PROFORMA_M set iscancel=1,cancelledby='" & Session("UserId_DepoCFS") & "',CancelledOn=getdate() where InvoiceNo='" & AssessNo & "' and WorkYear='" & WorkYear & "'"
    ...
    Protected Sub lnkIsubmit_Click(sender As Object, e As EventArgs)
        Try
            Dim gr As GridViewRow = CType(CType(sender, LinkButton).NamingContainer, GridViewRow)
            Dim lnkAnnexure As LinkButton = DirectCast(sender, LinkButton)
            Dim row As GridViewRow = DirectCast(lnkAnnexure.Parent.Parent, GridViewRow)
            Dim InvoiceNo As String = lnkAnnexure.CommandArgument
            Dim WorkYear As String = grdSummary.DataKeys(row.RowIndex).Value.ToString()
            Dim LineID As String = Trim(CType(gr.FindControl("lblLineID"), Label).Text)
            Dim InvoiceType As String = Trim(CType(gr.FindControl("lblInvoiceType"), Label).Text)
            Dim DeliveryType As String = Trim(CType(gr.FindControl("lblDeliveryType"), Label).Text)
            Dim AssessNo As String = Trim(CType(gr.FindControl("lblassessNo"), Label).Text)
            If DeliveryType = "O" Then


                strSql = ""
                strSql += "USP_INSERT_INTO_EYARD_ASSESSM_OTHER_Submit '" & InvoiceNo & "','" & WorkYear & "','" & InvoiceType & "','" & Session("UserId_DepoCFS") & "','" & AssessNo & "'"
                ds = db.sub_GetDataSets(strSql)

                btnsearch_Click(sender, e)
                lblsession.Text = "Assessment Saved successfully"
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModalforupdate2", "$('#myModalforupdate2').modal();", True)
                UpdatePanel6.Update()
                ClientScript.RegisterStartupScript(Page.GetType(), "OpenList", "<script>OpenCancelInvoice(); </script>")
            End If
