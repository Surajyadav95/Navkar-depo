...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Filldropdown()
        Try
            ...
            ddlLineCustomer.DataSource = ds.Tables(9)
            ddlLineCustomer.DataTextField = "CustomerName"
            ddlLineCustomer.DataValueField = "CustomerName"
            ddlLineCustomer.DataBind()
            ddlLineCustomer.Items.Insert(0, New ListItem("--Select--", 0))
    ...
    Protected Sub grid()
        Try
            ...
            End If
            If Val(ddlshipline.SelectedValue) > 0 Then


                If ddlLineCustomer.SelectedValue > 0 Then
                    txttotalamount.Text = ""
                Else
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "SELECT ContainerNo FROM EYARD_IN WHERE ContainerNo='" & Trim(txtcontainerNo.Text) & "' AND STATUS='P' AND ISCANCEL=0"
            dt = db.sub_GetDatatable(strSql)
            If (dt.Rows.Count > 0) Then
                txtcontainerNo.Text = ""
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql = "USP_CTR_STATUS'" & Trim(txtcontainerNo.Text & "") & "'"
            dt = db.sub_GetDatatable(strSql)
            If (dt.Rows.Count > 0) Then
                txtcontainerNo.Text = ""
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container No Already Gate in .');", True)
            ...
            'For Each row As GridViewRow In grdcontainer.Rows
            '    strSql = ""
            '    strSql += "USP_INSERT_EYARD_IN'" & intEntryID & "','" & intgateinid & "','" & Convert.ToDateTime(Trim(txtindatetime.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(ddlEntrytype.SelectedItem.Text & "") & "','" & Trim(ddlmovementby.SelectedItem.Text & "") & "',"
            ...
            '    strSql += "'" & Val(CType(row.FindControl("lblLineid"), Label).Text & "") & "','" & Trim(CType(row.FindControl("lblConsignee"), Label).Text & "") & "','" & Trim(CType(row.FindControl("lblCondition"), Label).Text & "") & "',"
            ...
            '    strSql += "'" & Replace(Trim(CType(row.FindControl("lblDamageRemarks"), Label).Text & ""), "'", "''") & "','" & Convert.ToDateTime(CType(row.FindControl("lblDoValidDate"), Label).Text.ToString()).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(chkisActive.Checked & "") & "','" & Session("UserId_DepoCFS") & "','" & Replace(Trim(CType(row.FindControl("lblCSCASP"), Label).Text & ""), "'", "''") & "'"
            ...
            '    Next
            'End If
            txtEyardInPrint.Text = Trim(dtEmptyIn.Rows(0)("GateInNo") & "")
            txtEyardRecipt.Text = Trim(dtEmptyIn.Rows(0)("GateInNo") & "")
            subClear()
    ...
    Protected Sub txtcontainerNo_TextChanged(sender As Object, e As EventArgs)
        Try
            If Not Trim(txtcontainerNo.Text & "") = "" Then
                ...
                dt = db.sub_GetDatatable(strSql)
                If dt.Rows.Count > 0 Then
                    ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('container already hold!');", True)
                    txtcontainerNo.Text = ""
                    txtcontainerNo.Focus()
                    Exit Sub
                End If
    ...
    Protected Sub ddlLineCustomer_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " select CustomerName from MNR_Customer_Line where CustomerName='" & Trim(ddlLineCustomer.SelectedValue) & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                txtShippConsignee.Text = Trim(dt.Rows(0)("CustomerName") & "")
    ...
    Protected Sub ddlshipline_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            If ddlshipline.SelectedValue = "216" Then


                divCusmer.Attributes.Add("style", "display:block")
                ddlLineCustomer.Focus()
            
            End If
            ddlstatusType.Focus()
            Call txtShippConsignee_TextChanged(sender, e)
    ...
    Protected Sub txtShippConsignee_TextChanged(sender As Object, e As EventArgs)
        Try
