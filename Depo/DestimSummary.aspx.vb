...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnShow_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "USP_GetDestimSummary '" & Convert.ToDateTime(Trim(txtfromDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(Trim(txttoDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & ddlBasedOn.SelectedValue & "','" & Trim(ddlSearchOn.SelectedItem.Text) & "',"
    ...
    Protected Sub ExportToExcel_Click(sender As Object, e As EventArgs) Handles ExportToExcel.Click
        Try
            ...
            End If
            strSql = ""
            strSql += "USP_GetDestimExporttoExcel '" & Convert.ToDateTime(Trim(txtfromDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(Trim(txttoDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & ddlBasedOn.SelectedValue & "','" & Trim(ddlSearchOn.SelectedItem.Text) & "',"
            If Trim(ddlSearchOn.SelectedItem.Text) = "Shipping Line" Then
                strSql += "'" & Trim(ddlShipline.SelectedItem.Text) & "'"
            ElseIf Trim(ddlSearchOn.SelectedItem.Text) = "Container No" Then
                strSql += "'" & Trim(TxtContainerNo.Text) & "'"
            ...
            If (dt.Rows.Count > 0) Then
                'Dim intCount As Integer = dt.Rows.Count
                Using wb As New XLWorkbook()
                    wb.Worksheets.Add(dt, "Destim Summary" & Convert.ToDateTime(Now).ToString("dd-MM-yyyy") & "")
                    With wb.Worksheets(0)
                    ...
                    Response.Charset = ""
                    Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    Response.AddHeader("content-disposition", "attachment;filename=DestimSummary" & Convert.ToDateTime(Now).ToString("ddMMyyyyhhmm") & ".xlsx")
                    Using MyMemoryStream As New MemoryStream()
                        wb.SaveAs(MyMemoryStream)
                        MyMemoryStream.WriteTo(Response.OutputStream)
                        Response.Flush()
