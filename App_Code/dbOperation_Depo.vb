...
Public Class dbOperation_Depo
    ...
    Public Sub sub_ConnectDB()
        Try
            ...
            strConnString = ConfigurationManager.ConnectionStrings("SqlConnString_Depo").ConnectionString
...
If conn.State = ConnectionState.Open Then conn.Close()
    ...
    Public Function GetExcelColumnName(columnNumber As Integer) As String
        ...
        Return columnName
    End Function
    Public Sub AmmendmentLog(ByVal Description As String, ByVal UserID As Integer)
        Dim strQuery As String = ""

        strQuery = "INSERT INTO ammendmentlog VALUES ( '" & Replace(Description, "'", "''") & "', '" & UserID & "','" & Convert.ToDateTime(Now).ToString("yyyy-MM-ddTHH:mm") & "','')"
        sub_ExecuteNonQuery(strQuery)
    End Sub
    Public Function InsertEstimationEntry(ByVal Component_Code As String, ByVal Location As String, ByVal DamageCode As String, ByVal Material_Type As String, ByVal TypeOfRepair As String,
                                           ByVal DescriptionOfRepair As String,
                                           ByVal Activity As String, ByVal Party As String, ByVal RepairCode As String, ByVal Dimension As String, ByVal Length As Double,
                                           ByVal Width As Double, ByVal Unit As String, ByVal Qty As Double, ByVal ManHrs As Double, ByVal ManCost As Double, ByVal MaterialCost As Double, ByVal TotalCost As Double,
                                           ByVal UserID As Integer, ByVal ContainerNo As String, ByVal EstimateType As String)
        Dim dtReturn As New DataTable
        Try
            sub_ConnectDB()
            Dim sqlCommandSting As String
            sqlCommandSting = "USP_INSERT_INTO_TEMP_GENERATE_ESTIMATES_OVMNR "
            Dim cmd As New SqlCommand(sqlCommandSting, conn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("@Component_Code", Component_Code)
            cmd.Parameters.AddWithValue("@Location", Location)
            cmd.Parameters.AddWithValue("@DamageCode", DamageCode)
            cmd.Parameters.AddWithValue("@Material_Type", Material_Type)
            cmd.Parameters.AddWithValue("@TypeOfRepair", TypeOfRepair)
            cmd.Parameters.AddWithValue("@DescriptionOfRepair", DescriptionOfRepair)
            cmd.Parameters.AddWithValue("@Activity", Activity)
            cmd.Parameters.AddWithValue("@Party", Party)
            cmd.Parameters.AddWithValue("@RepairCode", RepairCode)
            ...
            cmd.Parameters.AddWithValue("@ManHrs", ManHrs)
            cmd.Parameters.AddWithValue("@ManCost", ManCost)
            ...
            cmd.Parameters.AddWithValue("@EstimateType", EstimateType)


            'cmd.CommandType = CommandType.Text
            'cmd.CommandText = sqlCommandSting
            'cmd.Connection = conn
            'If conn.State = ConnectionState.Closed Then conn.Open()
            'cmd.ExecuteNonQuery()
            'cmd.Parameters.Clear()
